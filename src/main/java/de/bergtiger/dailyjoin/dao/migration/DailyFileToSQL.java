		...
		if (file.exists()) {
			try {
					...
					for (int i = 0; i < players.size(); i++) {
						try {
							...
							try {
								...
								TigerLogger.log(Level.SEVERE, "FileToSQL, no Connection and could not save back to file", e);
							}
							TigerConnection.noConnection();
							return;
						} catch (UpdatePlayerException e) {
							TigerLogger.log(Level.SEVERE, "FileToSQL", e);
						}
					}
				} else {
					// EmptyList
					TigerLogger.log(Level.INFO, "Empty Offline List");
				}
				...
				if (players == null || players.isEmpty()) {
					if(file.delete()) {
						// file deleted
						TigerLogger.log(Level.INFO, "file deleted");
					} else {
						// could not delete file
						TigerLogger.log(Level.INFO, "could not delete file");
					}
				}
			} catch (NoSQLConnectionException e1) {
				// should never be
				TigerLogger.log(Level.INFO, "that should never happen :°(", e1);
			}
		} else {
			// No File
			TigerLogger.log(Level.INFO, "No Offline List");
		}
	}
}
