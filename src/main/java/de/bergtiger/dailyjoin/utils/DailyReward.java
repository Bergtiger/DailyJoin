...
public class DailyReward {
	...
	private final HashMap<Integer, List<String>> rewardsConsecutive = new HashMap<>();
	private final HashMap<Integer, List<String>> rewardsTotal = new HashMap<>();
				...
				if (!keys.isEmpty()) {
					keys.forEach(k -> {
						try {
							...
							TigerLogger.log(Level.WARNING, String
									.format("loadDaysConsecutive: %s is not a valid number and will be ignored", k), e);
						}
					});
				} else {
					TigerLogger.log(Level.INFO, "loadDaysConsecutive: No Keys");
				}
			} else {
				TigerLogger.log(Level.INFO,
						String.format("loadDaysConsecutive: No file '%s'", file.getAbsolutePath()));
			}
		} else {
			TigerLogger.log(Level.WARNING, "loadDaysConsecutive: missing file in configuration");
		}
	}
		...
		if (DailyConfig.inst().hasValue(DailyConfig.FILE_DAYS_TOTAL)) {
			...
			if (file.exists()) {
				...
				if (!keys.isEmpty()) {
					...
					TigerLogger.log(Level.INFO, "loadDaysTotal: No Keys");
				}
			} else {
				TigerLogger.log(Level.INFO,
			...
			TigerLogger.log(Level.WARNING, "loadDaysTotal: missing file in configuratio");
		}
	}
	...
	private void giveDaysConsecutive(Player p, int day) {
		if (rewardsConsecutive.containsKey(day)) {
	...
	private void giveDaysTotal(Player p, int totaldays) {
		if (rewardsTotal.containsKey(totaldays)) {
	 ...
	 * 
	 * @param p {@link Player} who gets the reward
	 * @param totaldays to show in message
	 * @param t now
	 */
	...
	private void performCmds(Player p, List<String> list) {
		for (String s : list) {
			Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(),
					s.replace(Lang.PLAYER, p.getName()).replace("@p", p.getName()));
		}
	}
