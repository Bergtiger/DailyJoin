	...
	public static String formatted(Timestamp t, String format) {
		if(t != null) {
			try {
				...
				TigerLogger.log(Level.SEVERE, String.format("formated: could not format Timestamp(%s) with format(%s)", t, format),e);
			}
		} else {
			TigerLogger.log(Level.WARNING, "formated: Timestamp null");
		}
		return "-";
	}
}
