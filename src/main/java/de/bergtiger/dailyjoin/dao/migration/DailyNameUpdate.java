	...
	private void updatePlayers(CommandSender cs) {	
		...
		thread = new Thread(() -> {
			try {
				...
				for(int i = 0; i < players.getPageMax(); i++) {
					...
					for (DailyPlayer dp : players.pageSublist(i)) {
						...
						if (!currentName.equalsIgnoreCase(dp.getName())) {
							// needs update
							TigerLogger.log(Level.INFO, String.format("updated %s to %s", dp.getName(), currentName));
							// set name
					...
					// show process
					TigerLogger.log(Level.INFO, String.format("update Names (%s%%)", getProcessFormatted()));
					// check if hasNext
				...
				// finished update
				TigerLogger.log(Level.INFO, "finished update Names.");
				...
				TigerLogger.log(Level.WARNING, "interrupted updating Names.");
				cs.spigot().sendMessage(Lang.build(Lang.UPDATE_NAME_EXCEPTION.get().replace(Lang.VALUE, "Interrupted")));
			} catch (NoSQLConnectionException e) {
				TigerLogger.log(Level.SEVERE, "stopped updating Names, SQL Connection lost.", e);
				...
				TigerLogger.log(Level.SEVERE, "could not update Names.", e);
