...
public class DailyFile implements MyUtils{
	 ...
	 */
	public void save(DailyPlayer dp) {
		// load file
		...
		try{
			...
			//	dailyjoin.inst().getDailyReward().giveReward(p, daysConsecutive, daysTotal, t);
			//}
...
/*			List<String> list = cfg.getStringList("uuids");
			...
			for (int i = 0; i < list.size(); i++) {
				paths = "player." + list.get(i) + ".";
				DailyPlayer p = new DailyPlayer(cfg.getString(paths + "name"), list.get(i), cfg.getInt(paths + "day"),
						cfg.getInt(paths + "totaldays"), new Timestamp(cfg.getLong(paths + "firstjoin")),
						new Timestamp(cfg.getLong(paths + "lastjoin")));
				players.add(p);
			}*/
